...
// Main handler for hourly email scheduler
const handler: Handler = async (event, context) => {
  // Only allow POST requests (for manual triggering) or scheduled invocations
  ...
  try {
    console.log('📧 Hourly Email Scheduler Started');
    ...
    const result = {
      success: true,
      message: 'Hourly email scheduler completed',
      emailsSent,
      emailsSkipped,
      errors,
    ...
    console.error('❌ Critical error in hourly email scheduler:', error);
    return {
      statusCode: 500,
      body: JSON.stringify({ 
